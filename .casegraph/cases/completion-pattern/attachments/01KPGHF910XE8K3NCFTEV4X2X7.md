...
凍結 CLI には `cg case close` / `cg case archive` を含めません。
その代わり、現行 v0.1 では次の checklist で完了を表現します。
...
6. 必要なら `cg case show --case <caseId>` で counts と frontier summary を確認する

この flow を完了後に実行しても `case.state` は `open` のままで構いません。
`closed` / `archived` の明示 lifecycle command は later-phase の管理面で追加してよいものとします。
