...
- `task_run_regression`: 回帰テスト実行 (`estimate_minutes=45`)
- `task_update_notes`: リリースノート更新 (`estimate_minutes=15`)
- `task_submit_store`: ストア申請 (`estimate_minutes=20`)
- `task_monitor_post_release`: リリース後監視 (`estimate_minutes=30`)
...
- markdown checklist には ready な tasks を出す
- markdown summary には goals / waiting events も含める
...
cg node add --case release-1.8.0 --id task_run_regression --kind task --title "Run regression test" --state todo --metadata '{"estimate_minutes":45}'
cg node add --case release-1.8.0 --id task_update_notes --kind task --title "Update release notes" --state todo --metadata '{"estimate_minutes":15}'
cg node add --case release-1.8.0 --id task_submit_store --kind task --title "Submit to App Store" --state todo --metadata '{"estimate_minutes":20}'
cg node add --case release-1.8.0 --id task_monitor_post_release --kind task --title "Monitor post-release" --state todo --metadata '{"estimate_minutes":30}'
...
cg edge add --case release-1.8.0 --id e4 --type contributes_to --from task_run_regression --to goal_release_ready
cg edge add --case release-1.8.0 --id e5 --type contributes_to --from task_update_notes --to goal_release_ready
cg edge add --case release-1.8.0 --id e6 --type contributes_to --from task_submit_store --to goal_release_ready
cg edge add --case release-1.8.0 --id e7 --type contributes_to --from task_monitor_post_release --to goal_release_ready
