...
- 必要なら later-phase の migration 手段で materialized state を更新する

### 現行参照実装の migration surface
- `cg migrate check` は `.casegraph/workspace.yaml`, 各 `case.yaml`, 各 `events.jsonl` を走査し、
  現在の `SPEC_VERSION` (`0.1-draft`) と既知の migration path で扱えるかを返す
- 現行の既知 path は `0.0.9 -> 0.1-draft` のみ
- `cg migrate check --patch-file <path>` / `cg migrate run --patch-file <path>` で、
  明示指定した patch file も同じ migration surface に含められる
- `cg migrate run [--dry-run]` は workspace metadata / case metadata / explicit patch file の
  version marker を正規化し、必要な場合のみ cache rebuild を行う
- legacy event log は file rewrite せず、reducer の reader compatibility で扱う
- unsupported `spec_version` が見つかった場合、`cg migrate run` は
  `migration_unsupported_version` (exit code 2) で停止する
- `case.yaml` の `spec_version` は `0.1-draft` では optional とし、未記載は現行 version 扱いにする
- cache DB 自体は migration 対象外で、cache は `cg cache rebuild` による再構築を正規ルートとする
...
この 4 つは、Phase 0 で CLI surface に含める recovery/admin command です。
