...
- `cg migrate check` は `.casegraph/workspace.yaml`, 各 `case.yaml`, 各 `events.jsonl` を走査し、
  現在の `SPEC_VERSION` (`0.1-draft`) で扱えるかを返す
- `cg migrate run [--dry-run]` は現行では no-op で、`changed = false`, `applied_steps = []` を返す
...
- patch file migration と cache DB migration は現時点では対象外で、
  cache は `cg cache rebuild` による再構築を正規ルートとする
