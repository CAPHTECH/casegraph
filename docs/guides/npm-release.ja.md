...
OTP を 1 回渡して release 全体を publish する場合は次を使います。

```bash
pnpm publish:release -- --otp 123456
```

OTP の期限切れや後半 package の失敗時は、特定 package から再開できます。

```bash
pnpm publish:release -- --from @caphtech/casegraph-worker-shell --otp 654321
```

無人実行の publish では、npm trusted publishing か 2FA bypass 付き granular access token を優先してください。この OTP 補助はローカルでの対話的 release を前提にしています。

## Publish 順序

`pnpm publish:release` は CLI が依存パッケージを解決できるよう、依存順で公開します。実際の順序は次です。
