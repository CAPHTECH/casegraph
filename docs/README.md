...
特定のタスク管理 SaaS や特定の LLM ベンダーを前提にしません。外部 sync / sink は optional integration であり、core 成立の前提ではありません。
...
- optional な projection / sync protocol
- local-first ストレージ
- 最低限の reverse sync
...
- projection / sync は optional integration track とし、core roadmap completion の必須条件には含めない
