...
- AI worker patch extraction: `packages/core/src/worker-prompt.ts::buildAgentPrompt` + `extractPatchFromText` are the shared helpers for the `code-agent` and `local-llm` workers. Agents are expected to emit a single ```` ```casegraph-patch ```` fenced block (spec §7.10a) — worker-code-agent spawns a configured CLI and reads its stdout; worker-local-llm POSTs to Ollama `/api/generate` with `stream: true` and parses NDJSON chunks, appending each chunk to the worker-log file in real time and accumulating the full response before extraction. Both use the same extractor; if it fails the worker returns `status: "failed"` with no patch (never `worker_patch_invalid`).
- Worker-in-plugin retry: both AI workers retry once on extraction failure using `buildRetryPrompt({ originalPrompt, previousResponse, reason })`. The retry is contained inside the plugin, so worker-host sees a single `worker.execute` round-trip and `worker.dispatched`/`worker.finished` remain one-to-one. Observations include `Retry 1 after (<code>): <message>`; the worker-log file records both attempts under `## attempt N` sections.
